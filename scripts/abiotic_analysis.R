...
## Raw data is not independent, because there is replication of three TOMST loggers
# Will group together by making mean per day of the three TOMST

tomst_otc_temp_new <- tomst_otc_temp %>%
  mutate(date = as.Date(Datetime_UTC)) %>%
  group_by(Datetime_UTC, treatment_type) %>%
  summarise(
    Value = mean(Value)
  ) 
...
normality_raw_temp_otc_control <- tomst_otc_temp_new %>% 
  filter(treatment_type == "Control") 

ad.test(normality_raw_temp_otc_control$Value) # Not normal

normality_raw_temp_otc_treatment <- tomst_otc_temp_new %>% 
...
ggqqplot(tomst_otc_temp_new, x = "Value", facet.by = "treatment_type")
...
# Raw data test - Significant - W = 14624004, p-value < 2.2e-16
result_OTC_temp_raw <- wilcox.test(Value ~ treatment_type, data = tomst_otc_temp_new)
...
(temperature_plot <- ggplot() +
    geom_point(data = tomst_otc_temp_new, aes(x = as.Date(Datetime_UTC), y = Value, color = treatment_type), size = 0.5, alpha = 0.02) +
    geom_line(data = OTC_temp_stats, aes(x = date, y = max_temperature, group = treatment_type, color = treatment_type), size = 0.8, alpha = 0.8) +
    geom_line(data = OTC_temp_stats, aes(x = date, y = min_temperature, group = treatment_type, color = treatment_type), linetype = "dashed", size = 0.8, alpha = 0.8) +
...
# Raw Data conversion into usable dataset
...
# iButtons temperature ---- DID NOT USE IN THE FINAL MANUSCRIPT
...
# Wilcoxon test - non-significant - W = 911.5, p-value = 0.4392
result_OTC_hum_daily_max <- wilcox.test(max_humidity ~ treatment_type, data = daily_hum)

# Wilcoxon test - non-significant - W = 977, p-value = 0.2072
result_OTC_hum_daily_min <- wilcox.test(min_humidity ~ treatment_type, data = daily_hum)

# Plot for RH
# Adding missing dates
all_dates <- seq(min(OTC_temp_stats$date), max(OTC_temp_stats$date), by = "day")

daily_hum_new <- daily_hum
daily_hum_new <- daily_hum_new %>% 
  rename(date = day)
daily_hum_new$date <- as.Date(daily_hum_new$date)

daily_hum_new <- daily_hum_new %>%
  full_join(data.frame(date = all_dates), by = "date") %>%
  arrange(date)  # Ensure the data is ordered by date

(humidity_plot_na <- ggplot() +
    geom_point(data = humidity, aes(x = as.Date(datetime), y = value, color = treatment_type), size = 0.5, alpha = 0.02) +
    geom_line(data = daily_hum_new, aes(x = date, y = max_humidity, group = treatment_type, color = treatment_type), size = 0.8, alpha = 0.8) +
    geom_line(data = daily_hum_new, aes(x = date, y = min_humidity, group = treatment_type, color = treatment_type), size = 0.8,alpha = 0.8, linetype = "dashed") +
    scale_color_manual(values = my_colors) +  # Manually set colors
    theme_bw(base_size = 15) +
    
    # Set x-axis limits and specific breaks
    scale_x_date(limits = c(as.Date("2023-06-29"), as.Date("2023-08-29")),
                 breaks = c(as.Date("2023-06-29"), as.Date("2023-07-29"), as.Date("2023-08-29")),
                 labels = c("29-Jun", "29-Jul", "29-Aug")) +
...
# DID NOT USE IN THE FINAL ANALYSIS
...
# Plot for VPD
daily_vpd_new <- daily_vpd
daily_vpd_new <- daily_vpd_new %>% 
  rename(date = day)
daily_vpd_new$date <- as.Date(daily_vpd_new$date)
...
(vpd_plot <- ggplot() +
    geom_point(data = vpd, aes(x = as.Date(date), y = VPD, color = treatment_type), size = 0.5, alpha = 0.02) +
    geom_line(data = daily_vpd_new, aes(x = date, y = max_vpd, group = treatment_type, color = treatment_type), size = 0.8, alpha = 0.8) +
    geom_line(data = daily_vpd_new, aes(x = date, y = min_vpd, group = treatment_type, color = treatment_type), size = 0.8,alpha = 0.8, linetype = "dashed") +
    scale_color_manual(values = my_colors) +  # Manually set colors
    theme_bw(base_size = 15) +
...
# FIGURE 2
figure_2 <- {
  temperature_plot + humidity_plot_na + vpd_plot + plot_layout(ncol = 1)
} +
  plot_layout(nrow = 3, heights = c(10, 10))

ggsave("outcomes/microclimate/figure_2_new.png", plot = figure_2, width = 8, height = 7, dpi = 300)


# DID NOT USE IN THE FINAL MANUSCRIPT
...
### VPD COUNTS DAYS 0.3 kPa - DID NOT USE IN THE FINAL MANUSCRIPT
...
#### Raw data figures #### - DID NOT USE FOR THE FINAL MANUSCRIPT
...
light_OTC <- read.csv("data/microclimate/light_final.csv")

# light averages
average_per_OTC <- light_OTC %>% 
  group_by(in_out) %>% 
  summarise(mean_value = mean(light),
            SE = std.error(light))

# Light stats
shapiro.test(light_OTC$value)   # is normally distributed, p > 0.05
t.test(light ~ in_out, data = light_OTC) # t = 2.3081, df = 7.8093, p-value = 0.0506



# plot - did not use for the main manuscript
...
#### DID NOT USE THESE FOR THE MAIN MANUSCRIPT ----
