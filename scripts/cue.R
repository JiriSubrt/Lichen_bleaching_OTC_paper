...
figure_5 <- ggplot(cue_capped, aes(x = temperature, y = CUE, fill = treatment_type, color = treatment_type)) +
  
  # Add means with SEM as points with error bars (aligned, not dodged)
  ...
  }, geom = "pointrange", size = 1.5, alpha = 0.75, position = position_identity()) +
  
  # Increase point size
  stat_summary(fun = mean, geom = "point", size = 4, alpha = 0.75, position = position_identity()) +
  
  # Set colors manually
  scale_color_manual(values = colours) +
  
  # Optional: set fill too if using shapes that include fill
