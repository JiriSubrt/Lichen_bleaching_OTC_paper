...
# Data import
lrc <- read.csv("data/LRCs/lrc_data.csv")
...
ggsave("outcomes/light_curves/figure_S3.png", plot = LRC_plot, width = 12, height = 5, dpi = 300)
...
# Did not include Light compensation points in the final draft 
...
# lcp_merged <- read.csv('data/LRCs/light_compensation_points.csv')
...
# Light Saturation Points - extracted from modeled values ----
...
# Test whether there are differences between LSPs with temperature as a fixed effect
# Refit the model with lmerTest
model_lsp <- lmer(LSPcuv ~ treatment_type + (1 | temperature), data = all_lsp)
summary(model_lsp)

# Test whether there are differences between maxNP with temperature as a fixed effect --- DID NOT USE FOR THE FINAL ANALYSIS
# Refit the model with lmerTest
#model_maxNP <- lmer(maxNP ~ treatment_type + (1 | temperature), data = all_lsp)
...
# MEAN MAXIMAL RATES OF PS from Light curves ---- did not use for the final draft
