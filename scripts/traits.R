...
algal_area <- algal_layer_summary %>% 
  ...
  group_by(treatment_type, sample, individual_sample) %>% 
...
# model with lmerTest
model <- lmer(specific_algal_area ~ treatment_type + (1 | individual_sample), data = algal_area)
...
# Plot - not using for the final publication
...
# # Plots - not using for the publication
...
## Water holding capacity ---- Not using for the publication
